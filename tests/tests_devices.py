...
from services.devices.models.device_model import DeviceAddResponse, DeviceGet, DeviceUpdateResponse
...
class TestsDevices:
    ...
    def test_update_device(self, function_devices_api, function_create_device_api):
        response_id = function_create_device_api
        payload = generate_device_data()
        response = function_devices_api.update_device_api(payload.model_dump(by_alias=True), response_id)
        assert_status_code(response, 200)
